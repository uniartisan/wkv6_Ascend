...
public:
    ...
    {   
        ...
        //暂时只实现考虑 B * H能被核数整除的情况，不能整除时将remainder按照每个core一个head进行处理
        this->headPerCore = (this->B * this->H) / GetBlockNum();
        this->sizePerCore = this->headPerCore * T * N;
        uint32_t uh_offset = (this->headPerCore * GetBlockIdx()) % H;
        kGm.SetGlobalBuffer((__gm__ half*)k + this->sizePerCore * GetBlockIdx(), this->sizePerCore);
        vGm.SetGlobalBuffer((__gm__ half*)v + this->sizePerCore * GetBlockIdx(), this->sizePerCore);
        wGm.SetGlobalBuffer((__gm__ half*)w + this->sizePerCore * GetBlockIdx(), this->sizePerCore);
        rGm.SetGlobalBuffer((__gm__ half*)r + this->sizePerCore * GetBlockIdx(), this->sizePerCore);
        oGm.SetGlobalBuffer((__gm__ half*)o + this->sizePerCore * GetBlockIdx(), this->sizePerCore);

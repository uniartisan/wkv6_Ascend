...
def compare_outputs(B, T, HEADS, HEADS_DIM, data_type):
    ...
    q = generate_random_tensor(param_shape, data_type, low=-5, high=5)  # receptance
    k = generate_random_tensor(param_shape, data_type, low=-5, high=5)  # key
    v = generate_random_tensor(param_shape, data_type, low=-5, high=5)  # value
    w = generate_random_tensor(param_shape, data_type, low=-5, high=5)  # time_decay
    u = generate_random_tensor(u_shape, data_type, low=-5, high=5)  # time_first
    h = generate_random_tensor(h_shape, data_type, low=-5, high=5)  # initial_state
    # h = h.zero_()

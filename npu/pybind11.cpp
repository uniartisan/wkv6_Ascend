...
namespace my_wkv
{
    std::tuple<at::Tensor, at::Tensor> run_wkv6(uint32_t B, uint32_t T, uint32_t C, uint32_t H, 
                        ...
                        const at::Tensor &u, const at::Tensor &h_input)
    {
        ...
        __fp16 scale = 1.0 / sqrtf(C/H);

        ACLRT_LAUNCH_KERNEL(rwkv6_vector)
        (blockDim, acl_stream, B, T, C, H, scale, const_cast<void *>(k.storage().data()), const_cast<void *>(v.storage().data()),
